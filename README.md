...
| `CODEX_MODEL` / `CODEX_REASONING_EFFORT` | `gpt-5.6-luna` / `max` | 設定行が未作成の場合に使う Codex CLI の初期値。以後は「AI深掘り設定」画面の保存値を使う |
...
AI深掘りの外部ランナーは Codex CLI と `tm` CLI を組み合わせます。画面上部の「AI深掘り設定」から Codex CLI のモデル名と推論強度を保存できます。ランナーは各回の実行前に保存値を読み込み、次回以降の深掘りに適用します。初期値は `gpt-5.6-luna` と `max` です。task-manager サーバー自身は Codex CLI を起動せず、外部ランナーが `tm` 経由で受信・質問・提案を行います。
...
2. AI が現在の判断の分岐点をラウンドごとに出す。ラウンド数に上限はなく、前提が確定した質問だけを順番に提示する。判断が必要な質問にはクリック式の選択肢と AI のおすすめ・理由が表示され、最後に「その他（自由回答）」も選べる。回答・不明・AI への委任も選択できる。
